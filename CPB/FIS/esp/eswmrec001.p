...
ASSIGN c-arq-log-wms = "LOG_WMS_TST_" +
                   STRING(YEAR(TODAY),"9999") +
                   STRING(MONTH(TODAY),"99") + 
                   STRING(DAY(TODAY),"99") +                    
                   ".txt" .                   
...
REPEAT ON ERROR UNDO , RETRY
   ...
   /* /*Recebimento de Itens*/ /* nÆo utiliza */ 
    ...
    /*Confirma‡Æo Pagamento*/
    run pi-confirma-pagto.
...
procedure pi-confirma-pagto:

    for each esp-fatura-frete exclusive-lock where
             esp-fatura-frete.ind-sit-fatura = 3 and
             esp-fatura-frete.lg-pagto-conf  = no:
        ...
        find first tit_ap no-lock where
                   tit_ap.cdn_fornecedor = emitente.cod-emitente and
                   tit_ap.cod_tit_ap    = esp-fatura-frete.nr-fatura no-error.
        if avail tit_ap and
           tit_ap.dat_liquidac_tit_ap <> 12/31/9999 and 
           tit_ap.val_sdo_tit_ap       = 0          then
            run esp/eswms018.p (buffer tit_ap).
        else 
            next.
        
        if return-value = "OK" then
            assign esp-fatura-frete.lg-pagto-conf = yes.
    ...
    FOR EACH esp-fila-wms EXCLUSIVE-LOCK
       WHERE esp-fila-wms.data-hora-integracao = ? 
        ...
        CASE esp-fila-wms.cod-trans:
            ...
            WHEN "TransportadoraFIS" THEN DO:
                FIND transporte NO-LOCK WHERE
                     transporte.cod-transp = INTEGER(ENTRY(1,esp-fila-wms.chave,"|")) NO-ERROR.
                IF AVAIL transporte THEN
            ...
            when "ConfPagto" then do:
                find tit_ap no-lock where
                     tit_ap.cod_estab     = entry(1,esp-fila-wms.chave, "|")      and
                     tit_ap.num_id_tit_ap = int(entry(2,esp-fila-wms.chave, "|")) no-error.
