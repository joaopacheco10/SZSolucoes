...
*******************************************************************************/
{include/i-prgvrs.i ESWM005-G01 2.06.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_nat-operacao bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_nat-operacao 
...
DEFINE VARIABLE c_nat-operacao AS CHARACTER FORMAT "x(6)":U 
     LABEL "Natureza" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_nat-operacao AT ROW 1.88 COL 22.57 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 3.88 COL 2.14
     bt-cancela AT ROW 3.88 COL 13
     bt-ajuda AT ROW 3.88 COL 44
     rt-button AT ROW 3.63 COL 1
     SPACE(0.28) SKIP(0.02)
...
DO:
FIND esp-natur-frete NO-LOCK
    WHERE esp-natur-frete.nat-operacao EQ INPUT FRAME {&FRAME-NAME} c_nat-operacao NO-ERROR.

IF NOT AVAILABLE esp-natur-frete THEN DO:
    {utp/ut-table.i mgesp esp-natur-frete 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(esp-natur-frete).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_nat-operacao 
      WITH FRAME d-vapara.
  ENABLE rt-button c_nat-operacao bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESWM005-G01" "2.06.00.001"}
