...
def var i-rd-opc  as integer                      no-undo.
...
&Scoped-define FIELDS-IN-QUERY-br-resumo tt-resumo.nr-embarque tt-resumo.nr-resumo tt-resumo.nr-pedido tt-resumo.cod-emitente tt-resumo.nome-abrev tt-resumo.cod-usuario tt-resumo.dt-envio tt-resumo.enviado tt-resumo.confirmado tt-resumo.cancelado   
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-resumo w-livre _FREEFORM
  QUERY br-resumo NO-LOCK DISPLAY
      ...
      tt-resumo.confirmado FORMAT "Sim/NÆo":U
...
for each esp-resumo-wms no-lock where
        ...
        ((i-rd-opc = 3) or
         (i-rd-opc = 2  and not esp-resumo-wms.confirmado) or
         (i-rd-opc = 1  and esp-resumo-wms.confirmado)),
