...
*******************************************************************************/
{include/i-prgvrs.i ESCM104-G01 12.01.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-area i_tp-docto bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-area i_tp-docto 
...
DEFINE VARIABLE i_cod-area AS INTEGER FORMAT ">>>>>9":U INITIAL 0 
     LABEL "µrea" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE i_tp-docto AS INTEGER FORMAT ">9":U INITIAL 0 
     LABEL "Documento" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-area AT ROW 1.63 COL 30 COLON-ALIGNED WIDGET-ID 2
     i_tp-docto AT ROW 2.63 COL 30 COLON-ALIGNED WIDGET-ID 4
...
DO:
FIND es-acordo-area-docto NO-LOCK
    WHERE es-acordo-area-docto.cod-area EQ INPUT FRAME {&FRAME-NAME} i_cod-area
      AND es-acordo-area-docto.tp-docto EQ INPUT FRAME {&FRAME-NAME} i_tp-docto NO-ERROR.

IF NOT AVAILABLE es-acordo-area-docto THEN DO:
    {utp/ut-table.i mgesp es-acordo-area-docto 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-acordo-area-docto).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-area i_tp-docto 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-area i_tp-docto bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCM104-G01" "12.01.00.001"}
