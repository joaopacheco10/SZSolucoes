...
*******************************************************************************/
{include/i-prgvrs.i ESFT103 12.01.00.001}
...
def var h-conf-uf as handle no-undo.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnConectar w-cadsi2 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnDesconectar w-cadsi2 
...
DEFINE VARIABLE h_esft103-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esft103-v01 AS HANDLE NO-UNDO.
...
DEFINE BUTTON bt-atualiza 
     IMAGE-UP FILE "image/toolbar/im-globo-03.bmp":U
     IMAGE-DOWN FILE "image/toolbar/ii-globo-03.bmp":U
     IMAGE-INSENSITIVE FILE "image/toolbar/ii-globo-03.bmp":U
     LABEL "Button 1" 
     SIZE 4 BY 1.13.
...
DEFINE FRAME f-cad
     bt-atualiza AT ROW 1.38 COL 56.72 WIDGET-ID 2
     rt-button AT ROW 1.21 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 90 BY 16.08 WIDGET-ID 100.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-atualiza w-cadsi2
ON CHOOSE OF bt-atualiza IN FRAME f-cad /* Button 1 */
do:

    def var l-xml-conf        as longchar no-undo.
    def var l-xml-conf-ret    as longchar no-undo.
    def var c-cod-estabel-aux as char     no-undo.
    def var r-row-conf        as rowid    no-undo.

    run pi-get-rowid in h_esft103-v01 (output r-row-conf).

    find es-conf-uf-gnre no-lock where
         rowid(es-conf-uf-gnre) = r-row-conf no-error.

    if avail es-conf-uf-gnre then do:

        if not valid-handle(h-conf-uf) then
            run adapters/esaxsft003.p persistent set h-conf-uf.
    
        find first es-param-gnre no-lock no-error.
    
        fnConectar(es-param-gnre.cod-estabel).

        if h-server:connected() then
            run service/consultaUF.p on h-server (input es-conf-uf-gnre.cod-uf,
                                                  input es-conf-uf-gnre.cod-receita,
                                                  input es-param-gnre.cod-estabel,
                                                  output l-xml-conf-ret).

        fnDesconectar().

        if l-xml-conf-ret <> '' then
            run pi-le-retorno in h-conf-uf (input l-xml-conf-ret).

        RUN local-display-fields IN h_esft103-v01.


        run utp/ut-msgs.p (input 'show',
                           input 15825,
                           input 'Atualiza‡Æo Conclu¡da!').
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esft103-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esft103-v01 ).
       RUN set-position IN h_esft103-v01 ( 2.75 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 14.21 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esft103-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/esft103-z01.w,
                     ProgVaPara = esp/esft103-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_esft103-q01 ).
       RUN set-position IN h_esft103-q01 ( 1.00 , 63.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_esft103-v01. */
       RUN add-link IN adm-broker-hdl ( h_esft103-q01 , 'Record':U , h_esft103-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_esft103-v01 ).

       /* Links to SmartQuery h_esft103-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esft103-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esft103-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esft103-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esft103-v01 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFT103" "12.01.00.001"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnConectar w-cadsi2 
FUNCTION fnConectar RETURNS LOGICAL
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnDesconectar w-cadsi2 
FUNCTION fnDesconectar RETURNS CHARACTER
