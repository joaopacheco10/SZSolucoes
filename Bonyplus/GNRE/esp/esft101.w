...
*******************************************************************************/
{include/i-prgvrs.i ESFT101 12.01.00.001}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-financ 
...
DEFINE FRAME f-cad
     bt-financ AT ROW 1.38 COL 60.72 WIDGET-ID 2
     rt-button AT ROW 1.21 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 90 BY 15 WIDGET-ID 100.
...
&Scoped-define SELF-NAME bt-financ
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-financ w-cadsim
ON CHOOSE OF bt-financ IN FRAME f-cad /* Button 1 */
DO:

    def var r-param-gnre as rowid no-undo.

    run pi-rowid-param in h_esft101-v01 (output r-param-gnre).

    if r-param-gnre <> ? then do:

        current-window:sensitive = no.

        run esp/esft101a.w (input r-param-gnre).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esft101-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esft101-v01 ).
       RUN set-position IN h_esft101-v01 ( 2.79 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esft101-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/esft101-z01.w,
                     ProgVaPara = esp/esft101-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_esft101-q01 ).
       RUN set-position IN h_esft101-q01 ( 1.00 , 65.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_esft101-v01. */
       RUN add-link IN adm-broker-hdl ( h_esft101-q01 , 'Record':U , h_esft101-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_esft101-v01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-financ:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsim ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esft101-v01 ,
             bt-financ:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFT101" "12.01.00.001"}
